...
export const brevity: EvalCriterion = {
  name: 'brevity',
  ...
  async evaluate(input: EvalInput): Promise<EvalScore> {
    const maxChars =
      (input.metadata?.maxReviewChars as number) ?? DEFAULT_REVIEW_MAX_CHARS;
...
const actionabilityRubric: JudgeRubric = {
  criterion: 'Actionability',
  description:
    'Does the review contain concrete, specific next steps the reader can take? Not vague encouragement but real actions.',
  scale: [
...
export const actionability: EvalCriterion = {
  name: 'actionability',
  ...
  async evaluate(input: EvalInput, judge?: Judge): Promise<EvalScore> {
    if (!judge) throw new Error('Judge required for actionability criterion');
...
const honestyRubric: JudgeRubric = {
  criterion: 'Honesty',
  description:
    "Does the review honestly assess gaps and weaknesses? Does it avoid sugarcoating or false encouragement?",
  scale: [
...
export const honesty: EvalCriterion = {
  name: 'honesty',
  ...
  weight: 1.0,
  async evaluate(input: EvalInput, judge?: Judge): Promise<EvalScore> {
    if (!judge) throw new Error('Judge required for honesty criterion');
...
export const review: EvalCriterion[] = [brevity, actionability, honesty];
