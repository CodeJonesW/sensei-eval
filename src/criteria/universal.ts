...
const DEFAULT_LENGTH_LIMITS: Record<string, { min: number; max: number }> = {
...
export const formatCompliance: EvalCriterion = {
  ...
  async evaluate(input: EvalInput): Promise<EvalScore> {
    const issues: string[] = [];
    if (hasUnclosedCodeBlocks(input.content)) issues.push('unclosed code block');
    if (hasUnclosedBold(input.content)) issues.push('unclosed bold marker');
...
export const lengthCompliance: EvalCriterion = {
...
export const hasCodeBlockCriterion: EvalCriterion = {
  name: 'has_code_block',
...
export const hasStructure: EvalCriterion = {
  name: 'has_structure',
...
const engagementRubric: JudgeRubric = {
  criterion: 'Engagement',
  description:
    'Does the content hook the reader, maintain good pacing, and provide a satisfying closure?',
  scale: [
...
export const engagement: EvalCriterion = {
  name: 'engagement',
...
const repetitionRubric: JudgeRubric = {
  criterion: 'Repetition Avoidance',
  description:
    'Does this content differ meaningfully from the previously delivered content? Consider topic overlap, structure reuse, and phrasing similarity.',
  scale: [
...
export const repetitionAvoidance: EvalCriterion = {
...
export const universal: EvalCriterion[] = [
  formatCompliance,
  lengthCompliance,
