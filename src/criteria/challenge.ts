...
const problemClarityRubric: JudgeRubric = {
  criterion: 'Problem Clarity',
  description:
    'Is the problem statement unambiguous? Can the reader understand exactly what they need to build/solve?',
  scale: [
...
export const problemClarity: EvalCriterion = {
  ...
  async evaluate(input: EvalInput, judge?: Judge): Promise<EvalScore> {
    if (!judge) throw new Error('Judge required for problem_clarity criterion');
...
const difficultyCalibrationsRubric: JudgeRubric = {
  criterion: 'Difficulty Calibration',
  description:
    'Does the challenge match the expected difficulty level? Consider: concept complexity, number of steps, edge cases, and expected time to solve.',
  scale: [
...
export const difficultyCalibration: EvalCriterion = {
  ...
  weight: 1.0,
  async evaluate(input: EvalInput, judge?: Judge): Promise<EvalScore> {
    if (!judge) throw new Error('Judge required for difficulty_calibration criterion');
...
const hintQualityRubric: JudgeRubric = {
  criterion: 'Hint Quality',
  description:
    'Are hints provided? Are they progressive (nudge → more direct)? Do they help without giving away the answer?',
  scale: [
...
export const hintQuality: EvalCriterion = {
  name: 'hint_quality',
  ...
  weight: 1.0,
  async evaluate(input: EvalInput, judge?: Judge): Promise<EvalScore> {
    if (!judge) throw new Error('Judge required for hint_quality criterion');
...
const testabilityRubric: JudgeRubric = {
  criterion: 'Testability',
  description:
    'Can the solution be verified? Are there clear expected outputs, test cases, or validation criteria?',
  scale: [
...
export const testability: EvalCriterion = {
  name: 'testability',
  ...
  weight: 1.0,
  async evaluate(input: EvalInput, judge?: Judge): Promise<EvalScore> {
    if (!judge) throw new Error('Judge required for testability criterion');
...
export const challenge: EvalCriterion[] = [
  problemClarity,
